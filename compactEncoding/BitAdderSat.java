...
public class BitAdderSat {
  ...
  public static void main(String[] args) {
    ...
    int[][][] subwords = new int[maxSumLength][nVals][maxBits];
    for (int i=0; i<nVals; i++) {
      for (int j=0; j<nBitsNeeded[0]; j++) {
        subwords[0][i][j] = assign();
      }
    }
    for (int i=1; i<maxSumLength; i++) {
      for (int j=0; j<nVals-i; j++) {
        ...
        for (int k=0; k<nBitsNeeded[i]; k++) {
          AddOut sum = add(subwords[i-1][j][k],subwords[0][j+i][k],carryVar);
          if (sum.varAssign!=null)
            total.clauses.addAll(sum.varAssign.clauses);
          subwords[i][j][k] = sum.sumVar;
    ...
    for (int squareLen=1; squareLen<=maxSumLength; squareLen++) {
      for (int i=0; i<=nVals-2*squareLen; i++) {
        Formula f = forceInequality(subwords[squareLen-1][i], subwords[squareLen-1][i+squareLen], nBitsNeeded[squareLen-1]);
    ...
    if (mode==MAX_SYMBOL_MODE) {
      for (int i=0; i<nVals; i++) {
        Formula f = forceLessThanOrEqual(subwords[0][i], maxSymbol, nBitsNeeded[0]);
        if (f!=null) {
          total.clauses.addAll(f.clauses);
        }
      }
      ...
      for (int i=0; i<nVals; i++) {
        Formula f = forceInValidSet(subwords[0][i], valSet, nBitsNeeded[0]);
        if (f!=null) {
          total.clauses.addAll(f.clauses);
        }
      }
  ...
  static Formula forceInequality(int[] var1, int[] var2, int nBitsNeeded) {
  ...
  /**
   * Given the variable indices for the value at some position, produce a Formula
   ...
   */
  static Formula forceInValidSet(int[] var, int[] valSet, int nBitsNeeded) {
   ...
   */
  static Formula forceLessThanOrEqual(int[] var, int max, int nBitsNeeded) {
